...
// Function to find transitions for a given state and input
    ...
    while (front < rear) 
    {
        ...
        for (char input = '0'; input <= '1'; input++) 
        {
            ...
            if (nextState[0] != '\0') 
            {
                ...
                // Check if the new state has been added to the queue
                for (int i = 0; i < rear; i++) {
                    ...
                    if (i == rear - 1) 
                    {
                        // If we reached the end without finding it, add it to the queue
...
void printDFA() 
{
    ...
    // Check which DFA states are final states
    printf("\nFinal states in DFA:\n");
    for (int i = 0; i < dfaStateCount; i++) {
